...
public class LauncherView extends RelativeLayout {
    private int mHeight;
    private int mWidth;
    private int dp80 = CommonUtils.dp2px(getContext(), 80);
    private boolean mHasStart;
    private LauncherViewListener listener;

    public void setListener(LauncherViewListener listener) {
        this.listener = listener;
    }

    public LauncherView(Context context) {
        super(context);
        setWillNotDraw(false);//关于onDraw()方法不被执行的解决方法(setWillNotDraw)
    }
    ...
    private void init() {
        ...
        lp1.addRule(CENTER_HORIZONTAL, TRUE);//这里的TRUE 要注意 不是true
        lp1.addRule(CENTER_VERTICAL, TRUE);
        lp1.setMargins(0, 0, 0, 200);


//        test = new ImageView(getContext());
//        test.setLayoutParams(lp1);
//        test.setImageResource(R.drawable.shape_circle_red);
//        addView(test);
